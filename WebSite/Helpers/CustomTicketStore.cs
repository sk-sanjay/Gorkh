...
namespace WebSite.Helpers
{
    public class CustomTicketStore : ITicketStore
    {
        ...
        public async Task RemoveAsync(string key)
        {
            ...
            var TicketResult = await _httpClient.GetAsync("SiteAuthenticationTickets/Get", false, key).ConfigureAwait(false);
            var ticket = !string.IsNullOrEmpty(TicketResult) ? JsonConvert.DeserializeObject<SiteAuthenticationTicketsDTO>(TicketResult) : null;
            if (ticket != null)
                await _httpClient.DeleteAsync("SiteAuthenticationTickets/Delete", false, key).ConfigureAwait(false);
        }
        ...
        public async Task RenewAsync(string key, AuthenticationTicket ticket)
        {
            ...
            var _httpClient = scope.ServiceProvider.GetService<IHttpClientService>();
            var TicketResult = await _httpClient.GetAsync("SiteAuthenticationTickets/Get", false, key).ConfigureAwait(false);
            var authenticationTicket = !string.IsNullOrEmpty(TicketResult) ? JsonConvert.DeserializeObject<SiteAuthenticationTicketsDTO>(TicketResult) : null;
            if (authenticationTicket != null)
            {
                ...
                await _httpClient.PutAsync("SiteAuthenticationTickets/Edit", false, key, authenticationTicket).ConfigureAwait(false);
            }
        }
        ...
        public async Task<string> StoreAsync(AuthenticationTicket ticket)
        {
            ...
            var authenticationTicket = new SiteAuthenticationTicketsDTO
            {
                Id = userId,
                UserId = userId,
            ...
            var _httpClient = scope.ServiceProvider.GetService<IHttpClientService>();
            await _httpClient.PostAsync("SiteAuthenticationTickets/Upsert", false, authenticationTicket).ConfigureAwait(false);
