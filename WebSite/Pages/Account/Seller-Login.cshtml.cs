...
{
    [AllowAnonymous]
    public class Seller_LoginModel : PageModel
    {
        ...
        {
            Uri previousUrl = Request.GetTypedHeaders().Referer;
            if (previousUrl == null)
            {
            ...
            {
                string localurlpath = previousUrl.LocalPath;
                returnUrl ??= Url.Content(localurlpath);
            }            
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                HttpContext.Session.Remove("Glass");
                _notyf.Error("Invalid login attempt.<br/>Possible reasons:<br/>1. User not found, not approved, or not active.<br/>2. Password incorrect.<br/>3. Account lockedout.");
                return RedirectToPage();
            }
