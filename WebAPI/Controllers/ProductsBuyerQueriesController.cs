...
namespace WebAPI.Controllers
{
    ...
    public class ProductsBuyerQueriesController : Controller
    {
        private readonly IDataService _dataService;
        private readonly IEmailService _emailService;
        private readonly IRandomService _randomService;
        private readonly IFileService _fileService;
        public ProductsBuyerQueriesController(
        ...
        // GET ProductsBuyerQueries
        [HttpGet]
        public async Task<IActionResult> Get()
        {
            var modelVms = await _dataService.ProductsBuyerQueries.Get().ConfigureAwait(false);
            if (modelVms == null || modelVms.Count <= 0) return NotFound("Products Buyer Queries not found");
            return Ok(modelVms);
        }
        // GET ProductsBuyerQueries/5
        [HttpGet("{id}")]
        public async Task<IActionResult> Get([FromRoute] int id)
        {
            if (id <= 0) return BadRequest("Input not valid or null");
            var modelDto = await _dataService.ProductsBuyerQueries.Get(id).ConfigureAwait(false);
            if (modelDto == null) return NotFound("Products Buyer Queries not found");
            return Ok(modelDto);
        }
        // POST: ProductsBuyerQueries/Create
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] ProductsBuyerQueriesDTO inputModel)
        {
            ...
            var modelDto = await _dataService.ProductsBuyerQueries.Create(inputModel).ConfigureAwait(false);
        ...
        // PUT: ProductsBuyerQueries/Edit/5
        [HttpPut("{id}")]
        public async Task<IActionResult> Edit([FromRoute] int id, [FromBody] ProductsBuyerQueriesDTO inputModel)
        {
            if (inputModel == null) return BadRequest("Input not valid or null");
            if (id != inputModel.Id) return BadRequest("Invalid Id");
            if (!ModelState.IsValid) return BadRequest(ModelState.GetErrorMessages());

            var modelDto = await _dataService.ProductsBuyerQueries.Update(inputModel).ConfigureAwait(false);
            if (modelDto != null) return Ok(modelDto);
            return BadRequest("Update failed");
        }
        // DELETE: ProductsBuyerQueries/Delete/5
        [HttpDelete("{id}")]
        public async Task<IActionResult> Delete([FromRoute] int id)
        {
            if (id <= 0) return BadRequest("Input not valid or null");
            var rowsChanged = await _dataService.ProductsBuyerQueries.Delete(id).ConfigureAwait(false);
        ...
        [HttpGet("{buyerid}")]
        public async Task<IActionResult> GetProductsByBuyer([FromRoute] int buyerid)
        {
            var dropDownVms = await _dataService.ProductsBuyerQueries.GetProductsByBuyer(buyerid).ConfigureAwait(false);
            if (dropDownVms == null || dropDownVms.Count <= 0) return NotFound("Products not found");
            return Ok(dropDownVms);
        }
        //get products query by pid
        [AllowAnonymous]
        [HttpGet("{ProductId}/{buyerid}")]
        public async Task<IActionResult> GetProductsBuyerQueriesByPid(int ProductId, int buyerid)
        {
            var modelVms = await _dataService.ProductsBuyerQueries.GetProductsBuyerQueriesByPid(ProductId, buyerid).ConfigureAwait(false);
            if (modelVms == null) return NotFound("Products not found");
            return Ok(modelVms);
        }
        [AllowAnonymous]
        [HttpGet]
        public async Task<IActionResult> GetProductsBuyerQueriesForAdmin()
        {
            var dropDownVms = await _dataService.ProductsBuyerQueries.GetProductsBuyerQueriesForAdmin().ConfigureAwait(false);
            if (dropDownVms == null || dropDownVms.Count <= 0) return NotFound("Products Query not found");
            return Ok(dropDownVms);
        }
        [AllowAnonymous]
        [HttpGet("{id}")]
        public async Task<IActionResult> GetProductsBuyerQueriesById([FromRoute] int id)
        {
            if (id <= 0) return BadRequest("Input not valid or null");
            var modelDto = await _dataService.ProductsBuyerQueries.GetProductsBuyerQueriesById(id).ConfigureAwait(false);
            if (modelDto == null) return NotFound("Products and Buyer details not found");
            return Ok(modelDto);
        }
        [AllowAnonymous]
        [HttpGet("{ProductId}")]
        public async Task<IActionResult> GetProductsDetailsById([FromRoute] int ProductId)
        {
            if (ProductId <= 0) return BadRequest("Input not valid or null");
            var modelDto = await _dataService.ProductsBuyerQueries.GetProductsDetailsById(ProductId).ConfigureAwait(false);
            if (modelDto == null) return NotFound("Products details not found");
            return Ok(modelDto);
        }
    }
}
